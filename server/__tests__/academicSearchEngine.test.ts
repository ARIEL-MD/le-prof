...
describe("Academic Search Engine (100% sans IA)", () => {
  ...
  it("trouve un cours officiel du programme ivoirien (ex: Pythagore, SVT, PC)", async () => {
    ...
    assert.ok(res.coreConceptsAndFormulas.length > 0);
  ...
  it("gère de manière générale toute requête avec extraction de sujet et intention", async () => {
    ...
    assert.ok(res);
    assert.ok(res.definitionAndScope.length > 30);
    assert.ok(res.coreConceptsAndFormulas.length > 0);
  });
  ...
  it("répond précisément à une recherche de personnalité/auteur avec sa fiche dédiée", async () => {
    ...
    assert.ok(res.coreConceptsAndFormulas.some(c => /Caverne|Monde Intelligible|Idées|Réminiscence/i.test(c.name + " " + c.explanation)));
    assert.match(res.quickRevisionMemo, /Platon/i);
  });

  it("trouve précisément la fiche dédiée pour d'autres auteurs canoniques", async () => {
    const resSocrate = await searchAcademicCourseUnified({ query: "qui était socrate" });
    ...
    assert.ok(resSocrate.coreConceptsAndFormulas.some(c => /maïeutique/i.test(c.name + " " + c.explanation)));
    const resDescartes = await searchAcademicCourseUnified({ query: "descartes" });
  ...
  it("ne génère pas de mention boilerplate MENA/DPFC dans les explications", async () => {
    const resMaths = await searchAcademicCourseUnified({ query: "théorème de Pythagore", discipline: "mathematiques", level: "3e" });
    assert.ok(resMaths);
    ...
    const resPhilo = await searchAcademicCourseUnified({ query: "citations sur la liberté" });
    assert.ok(resPhilo);
...
describe("Recherche de Cours & Notions — anti faux positifs", () => {
  it("résout definition guerre froide dans le cours d'Histoire", async () => {
  ...
  it("résout plusieurs intentions sur une même notion sans changer de matière", async () => {
    ...
    for (const query of queries) {
      ...
      assert.equal(res.discipline, "histoire", query);
      assert.match(res.chapterTitle, /guerre froide/i, query);
    }
  });
  ...
  it("retourne le corpus demandé pour argument sur la liberté", async () => {
  ...
  it("refuse une fiche sans rapport", async () => {
    const res = await searchAcademicCourseUnified({ query: "xyzqv notion 847291" });
    assert.equal(res.noResult, true);
    assert.match(res.chapterTitle, /aucun résultat pertinent/i);
  });

  it("ne confond pas une requête à plusieurs termes avec une fiche partageant seulement un mot", async () => {
...
describe("Recherche universelle — intentions et sujets inédits", () => {
  ...
  it("comprend des formulations inédites sans dépendre d'une liste de requêtes", async () => {
    const res = await searchAcademicCourseUnified({ query: "pourquoi la chute du mur de Berlin a-t-elle marqué la fin de la guerre froide" });
    assert.equal(res.noResult, undefined);
    assert.match(res.chapterTitle, /Berlin|guerre froide/i);
    assert.ok(res.definitionAndScope.length > 50);
  });

  it("couvre plusieurs domaines avec la même logique de recherche", async () => {
    const cases = [
      { q: "définition de la mitose", title: /mitose/i },
      { q: "mécanisme de la photosynthèse", title: /photosynthèse|photosynthese/i },
      // Formulation suffisamment contextualisée pour sélectionner le contenu ONU pertinent.
      { q: "rôle de l'ONU dans le maintien de la paix", title: /ONU/i },
